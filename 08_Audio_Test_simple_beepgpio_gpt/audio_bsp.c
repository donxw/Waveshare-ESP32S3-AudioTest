...
#include "driver/uart.h" // ADDED: allow restoring UART0 pin mapping
#include "driver/gpio.h" // ADDED: provides GPIO_NUM_1 / GPIO_NUM_3
...
void audio_bsp_init(void)
{
  ...
  // Restore UART0 to the standard pins (GPIO1 TX, GPIO3 RX).
  // Some codec/init code remaps pins for I2S and may have affected UART0 routing.
  // This makes Arduino Serial (Serial.println) work reliably again.
...
void audio_play_init(void)
{
    ...
    // Also restore UART here in case audio_play_init is called separately
    uart_set_pin(UART_NUM_0, GPIO_NUM_1, GPIO_NUM_3, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
}
